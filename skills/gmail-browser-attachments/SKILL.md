...
- never persist the concrete password in manifests; only keep redacted `password_source`, `secret_scope`, `secret_purpose`, and `secret_persistence`
- keep portal login passwords and patient gate hints out of the PDF unlock lane; those use `portal_login:*` / `portal_patient_gate:*` secret ids, while PDF text extraction uses `pdf_unlock:*`
