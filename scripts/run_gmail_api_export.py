...
def resolve_full_messages(service: Any, locator: str, max_results: int) -> list[dict[str, Any]]:
    ...
    if message_id or (is_probable_api_id(locator) and not thread_id):
        ...
        return [
            service.users().messages().get(userId="me", id=resolved_id, format="full").execute()
        ]
    if thread_id:
...
def process_target(
    service: Any, target: Target, run_dir: Path, max_results: int
    ...
    row = {
        ...
        "ocr_manifest": "-",
        "pdf_text_manifest": str(pdf_text_manifest.resolve())
        if pdf_text_manifest.exists()
        else "-",
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Export Gmail-native attachments through the Gmail API."
    )
    parser.add_argument(
        "targets_tsv",
        help="TSV rows: <gmail query|message:<api_id>|thread:<api_id>><TAB><needle><TAB><mode?>",
    )
    parser.add_argument(
        "run_dir", nargs="?", default=f"runs/api-run-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
    )
    parser.add_argument(
        "--token",
        default=os.environ.get(
            "GMAIL_LAB_GOOGLE_TOKEN", "~/.gmail-lab/tokens/gmail-api-token.json"
        ),
    )
    parser.add_argument(
        "--client-secrets", default=os.environ.get("GMAIL_LAB_GOOGLE_CLIENT_SECRET", "")
    )
    parser.add_argument(
        "--no-browser",
        action="store_true",
        help="Print OAuth URL and ask for code instead of opening a browser",
    )
    ...
    explicit_client_secrets = Path(args.client_secrets) if args.client_secrets else None
    client_secrets_path = resolve_client_secrets_path(
        root=root,
        explicit=explicit_client_secrets,
    )
    try:
